...
function renderShell() {
  ...
  const todayIndex = 0;
  const precipitationChance = weather.daily.precipitation_probability_max?.[todayIndex];
  ...
  els.heroHighLow.textContent = `H ${formatTemperature(weather.daily.temperature_2m_max[todayIndex])}  L ${formatTemperature(weather.daily.temperature_2m_min[todayIndex])}`;
  els.currentCondition.textContent = weatherLabel(current.weather_code);
  els.heroSummary.textContent = buildHeroSummary(current, weather.daily);
  ...
  const hourlySlice = weather.hourly.time.slice(startIndex, startIndex + 24).map((time, index) => ({
    ...
    precipitationProbability: weather.hourly.precipitation_probability[startIndex + index]
  }));
  ...
  els.dailyForecast.innerHTML = weather.daily.time.slice(0, 14).map((date, index) => `
    <article class="forecast-card daily-card">
      ...
      <div class="forecast-note">${weather.daily.precipitation_probability_max[index] != null ? `${Math.round(weather.daily.precipitation_probability_max[index])}% precip` : ""}</div>
    </article>
  `).join("");
}
